  ...
  return (
    <div className="space-y-6">
      ...
      <GlassPanel className="p-5 space-y-3">
        <div className="font-mono text-[10px] tracking-widest" style={{ color: 'var(--acc2)' }}>
          THE OTHER INTELLIGENCE LAYER · OWN MODELS
        </div>
        <div className="text-xs leading-relaxed" style={{ color: 'var(--dim)' }}>
          The core thinks through a provider mesh; the own-model family computes with zero network. Both live in the same app — pin any own model in Chat, or call them over <span className="font-mono">/api/localmodels/chat</span>.
        </div>
          ...
          {OWN_MODELS.map((m) => (
            <div key={m.id} className="glass-2 rounded-xl p-3.5">
              <div className="flex items-center gap-2">
                ...
                <span className="chip !text-[8px] ml-auto" style={{ color: 'var(--acc2)' }}>{m.tagline}</span>
              </div>
