'use client';
// Aetherion — Projects: the AI-powered To-Do system (Puter KV or local),
...
function TasksPanel({ projectId, projectName }: { projectId: string; projectName: string }) {
  ...
  const runPlan = async () => {
    ...
    const id = tr.end();
    setPlan({ tasks: res.tasks, source: res.source === 'llm' ? `LLM (${res.model})` : 'Aetherion Local (offline template)' });
    setPlanning(false);
