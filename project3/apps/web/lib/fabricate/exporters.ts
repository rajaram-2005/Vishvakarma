// SUTRA Fabricate — mesh exporters. Pure functions: OBJ (text) and GLTF 2.0
...
export function meshToObj(mesh: Mesh, name = 'sutra_object'): string {
  ...
  const lines: string[] = [`# SUTRA Fabricate — ${name}`, `o ${name}`];
...
export function meshToGltf(mesh: Mesh, name = 'sutra_object'): string {
  ...
  const gltf = {
    asset: { version: '2.0', generator: 'SUTRA Fabricate' },
    scene: 0,
    scenes: [{ name, nodes: [0] }],
