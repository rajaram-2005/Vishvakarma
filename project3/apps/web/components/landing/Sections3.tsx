'use client';
// Lumen landing — MCP · GitHub · n8n · IDE · Browser · Autonomous Development.
...
export function McpSection() {
  return (
    <LandingSection
      ...
      sub="Discover servers, install with explicit scopes, monitor health, audit every tool call and pin versions. Lumen ships local adapters (filesystem, fetch) and speaks to remote MCP servers over stdio or HTTP."
    >
        ...
        {(repo || err) && !loading && (
          <div className="grid md:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {(repo?.compat ?? ['Lumen adapter coverage: high']).map((c) => (
                  <div key={c} className="text-xs flex items-center gap-2">
...
export function N8nSection() {
  return (
    <LandingSection
      ...
      sub="Workflows, templates, triggers, schedules, webhooks and AI/agent workflows. Lumen speaks n8n's format natively: author here, export standard n8n workflow JSON, or import their templates. The adapter respects n8n's terms of service — self-hosted or your own instance."
    >
      ...
      <div className="glass !rounded-[22px] overflow-hidden">
        <div className="flex border-b" style={{ borderColor: 'var(--line)' }}>
          ...
          <div className="flex-1 p-4 font-mono text-[10.5px] leading-relaxed" style={{ color: 'var(--dim)' }}>
            <div><span style={{ color: '#c084fc' }}>import</span> {'{ LumenCore }'} <span style={{ color: '#c084fc' }}>from</span> <span style={{ color: '#67e8f9' }}>"./core"</span>;</div>
            <div>&nbsp;</div>
            <div><span style={{ color: '#c084fc' }}>export default function</span> <span style={{ color: '#fbbf24' }}>App</span>() {'{'}</div>
            <div>&nbsp;&nbsp;<span style={{ color: '#c084fc' }}>const</span> core = <span style={{ color: '#c084fc' }}>new</span> <span style={{ color: '#fbbf24' }}>LumenCore</span>({'{ mode: '}<span style={{ color: '#67e8f9' }}>"local"</span>{' }'});</div>
            <div>&nbsp;&nbsp;core.<span style={{ color: '#fbbf24' }}>connect</span>();</div>
            <div>&nbsp;&nbsp;<span style={{ color: '#c084fc' }}>return</span> (&lt;main&gt;&lt;h1&gt;Lumen&lt;/h1&gt;&lt;/main&gt;);</div>
            <div>{'}'}</div>
          </div>
    ...
    >
      <div className="glass-2 p-5 max-w-2xl">
        ...
        <div className="space-y-2 font-mono text-[10.5px]" style={{ color: 'var(--dim)' }}>
          ...
          <div><span style={{ color: 'var(--acc2)' }}>› verify</span> "Lumen" visible in viewport ✓</div>
        </div>
      </div>
    </LandingSection>
  );
