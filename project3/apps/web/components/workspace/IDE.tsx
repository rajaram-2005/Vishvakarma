'use client';
// SUTRA IDE — File Tree | Code Editor | AI Agent
...
export function IDE() {
  ...
  const [termLines, setTermLines] = useState<string[]>(['SUTRA sandbox terminal — type "help". Everything dangerous is policy-gated.']);
  ...
  const [browserUrl, setBrowserUrl] = useState('sutra://local/preview');
  ...
  const runPipeline = async () => {
    ...
    const steps: Array<{ name: string; fn: () => void | Promise<void> }> = [
      ...
      },
      {
        name: 'Coder',
        fn: () => {
          const code = `// generated by SUTRA autonomous pipeline
// goal: ${goal}
...
`;
      ...
      {
        name: 'Browser',
        fn: () => setPipelineLog((l) => [...l, '    preview sutra://local/preview → console clean · screenshot attached']),
      },
      {
        name: 'Tester',
        fn: () => {
