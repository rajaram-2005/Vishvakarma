...
> Since the Aetheris merge, the whole product (UI + embedded core + own-model API)
> ships as ONE app. The one-container deploy path is `project3/Dockerfile`
> (+ `render.yaml`, `fly.toml`, see `DEPLOY.md`). The multi-service manifests in
> this folder are kept for the split-architecture deployments.
